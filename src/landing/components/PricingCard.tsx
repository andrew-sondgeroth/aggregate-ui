  ...
  return (
    <div
      className={`rounded-xl border p-8 flex flex-col ${
        highlighted
          ? 'border-[var(--color-accent-blue)] bg-[var(--color-bg-card)] shadow-lg shadow-[var(--color-accent-blue)]/10'
          : 'border-[var(--color-border)] bg-[var(--color-bg-card)]'
      }`}
    >
      {highlighted && (
        <div className="text-xs font-semibold text-[var(--color-accent-blue)] uppercase tracking-wider mb-4">
          Most Popular
        </div>
      )}
      ...
      <div className="mt-2 text-sm text-[var(--color-text-secondary)]">{limit}</div>
      ...
      <a
        href="#"
        className={`mt-8 block text-center rounded-lg px-6 py-3 font-semibold transition ${
          highlighted
