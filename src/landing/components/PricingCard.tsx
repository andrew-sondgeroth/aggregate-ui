...
export default function PricingCard({ tier, price, limit, features, highlighted }: PricingCardProps) {
  return (
    <div
      className={`rounded-2xl border p-8 sm:p-10 flex flex-col ${
        highlighted
          ? 'border-[var(--color-accent-blue)] bg-[var(--color-bg-card)] shadow-lg shadow-[var(--color-accent-blue)]/10 relative scale-[1.02]'
          : 'border-[var(--color-border)] bg-[var(--color-bg-card)]'
      }`}
    >
      {highlighted && (
        <div className="inline-flex self-start rounded-full bg-[var(--color-accent-blue)]/10 px-3 py-1 text-xs font-semibold text-[var(--color-accent-blue)] uppercase tracking-wide mb-6">
          Most Popular
        </div>
      )}
      <h3 className="text-xl font-bold text-[var(--color-text-primary)]">{tier}</h3>
      <div className="mt-5">
        <span className="text-5xl font-bold text-[var(--color-text-primary)]">{price}</span>
        <span className="ml-2 text-[var(--color-text-muted)]">/month</span>
      </div>
      ...
      <a
        href="#"
        className={`mt-10 block text-center rounded-lg px-6 py-4 font-semibold transition ${
          highlighted
